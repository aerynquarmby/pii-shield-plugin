...
// Turning the prompt guard to `warn` must not be a way to keep using the product unpaid: `warn`
// still prints the detection reasoning, which is the thing being paid for. `off` is different —
// it exits at the top of this file, before any of this runs, and leaves the user with nothing.
// That is not a billing bypass, so it is offered here.
//
// Offering it matters. UserPromptSubmit has no "allow anyway" affordance in Claude Code — there
// is no ask/approve decision for this event, only block — so a message that lists no alternative
// leaves someone stuck mid-task with no idea what to do next.
const gate = await resolveGate();
if (!gate.open) {
  process.stdout.write(JSON.stringify({
    decision: "block",
    reason: `${blockedMessage(gate, "This prompt")}\n\nDetected in:\n${summary}\n\n` +
      `To carry on without the prompt guard, set PII_SHIELD_PROMPT_MODE=off — that turns this ` +
      `check off entirely, so nothing in your prompts is checked or protected. If this was ` +
      `flagged wrongly, please send us the prompt: nothing personal is transmitted, only the ` +
      `types listed above.`,
  }));
  process.exit(0);
}
